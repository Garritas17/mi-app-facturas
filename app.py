import pdfplumber
import pandas as pd
import os
import re

def limpiar_valor(v):
    if v is None or v == "": return 0.0
    limpio = re.sub(r'[^\d.]', '', str(v).replace(',', ''))
    try: return float(limpio)
    except: return 0.0

def extraer_datos_baolai(ruta_pdf):
    items_finales = []
    try:
        with pdfplumber.open(ruta_pdf) as pdf:
            texto_cabecera = pdf.pages[0].extract_text()
            
            # Búsqueda de Factura (Captura BLAXXXXXX)
            factura_match = re.search(r'BLA\d+[A-Z0-9]*', texto_cabecera)
            val_factura = factura_match.group(0) if factura_match else "No Identificado"
            
            # Búsqueda de Fecha y RUC
            fecha_match = re.search(r'(?:FECHA|DATE)[:\s]*([\d\w/-]+)', texto_cabecera, re.IGNORECASE)
            ruc_match = re.search(r'RUC[:\s]*(\d+)', texto_cabecera)
            
            val_fecha = fecha_match.group(1) if fecha_match else "N/A"
            val_ruc = ruc_match.group(1) if ruc_match else "N/A"

            for page in pdf.pages:
                tablas = page.extract_tables()
                for tabla in tablas:
                    for fila in tabla:
                        if fila and fila[0] and str(fila[0]).strip().isdigit():
                            fila_l = [str(c).replace('\n', ' ').strip() if c else "" for c in fila]
                            
                            # Captura de precios y totales (últimas columnas)
                            monto_total = fila_l[-1] 
                            precio_unit = fila_l[-2] 

                            items_finales.append({
                                "Invoice_No": val_factura,
                                "Date": val_fecha,
                                "RUC_Buyer": val_ruc,
                                "Item_No": fila_l[0],
                                "Products": fila_l[1],
                                "Grade": fila_l[2] if len(fila_l) > 2 else "",
                                "Diameter": fila_l[3] if len(fila_l) > 3 else "",
                                "Wall_Thickness": fila_l[4] if len(fila_l) > 4 else "",
                                "Length_M": fila_l[5] if len(fila_l) > 5 else "",
                                "KGS_PC": limpiar_valor(fila_l[6]) if len(fila_l) > 6 else 0,
                                "TTL_PCS": limpiar_valor(fila_l[12]) if len(fila_l) > 12 else 0,
                                "TTL_KGS": limpiar_valor(fila_l[13]) if len(fila_l) > 13 else 0,
                                "PRECIO_UNITARIO_USD": limpiar_valor(precio_unit),
                                "VALOR_TOTAL_USD": limpiar_valor(monto_total)
                            })
    except Exception as e:
        print(f"Error procesando {ruta_pdf}: {e}")
    return items_finales

if __name__ == "__main__":
    resultados = []
    # Busca archivos en la carpeta principal
    archivos_pdf = [f for f in os.listdir('.') if f.lower().endswith(".pdf")]
    
    for arc in archivos_pdf:
        print(f"Procesando: {arc}")
        data = extraer_datos_baolai(arc)
        resultados.extend(data)

    if resultados:
        df = pd.DataFrame(resultados)
        df.to_excel("reporte_final.xlsx", index=False)
        print("Archivo reporte_final.xlsx generado con éxito.")
    else:
        print("No se encontraron datos para exportar.")
